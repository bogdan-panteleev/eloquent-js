...
const specialFormCompileMatcher = {
	...
	if: function compileIf(ast) {
		return `${compileExpression(ast.args[0])} ? ${compileExpression(ast.args[1])} : ${compileExpression(
			ast.args[2]
		)}`;
	},
...
const topScopeCompileMatcher = {
	print: function compilePrint(ast) {
		const input =
			ast.args[0].type === 'value' && typeof ast.args[0].value === 'string'
				? `"${compileExpression(ast.args[0])}"`
				: `${compileExpression(ast.args[0])}`;
